...
2. Исправляй очевидные ошибки распознавания в названиях чая ("Шэньпээр" → "Шэн пуэр") и в дегустационных словах: «грачит», «горчить» → «горчит»; «терпкий», «вяжет», «покалывает» — обычные слова о вкусе, оставляй как есть. Слово «пролив» распознаётся хорошо, а вот номер рядом с ним — плохо; см. правило 3a.
...
3a. Строка с пометкой «[Вся сессия]» — это ОДНА непрерывная запись всей дегустации, а не заметка одного пролива. Раздели её на проливы сам, по номерам, которые называет говорящий: «первый пролив», «пролив два», «3 пролив», «в четвёртом проливе» — всё это границы. Номер может быть распознан криво («в простом проливе» — это «в шестом проливе»); если по порядку видно, какой это пролив, восстанови его. Из «[Вся сессия]» ничего не переноси в description чая, кроме прямого итога обо всём чае. К проливам, выделенным из «[Вся сессия]», правила 4–9 применяются В ТОЧНОСТИ так же, как к помеченным: у каждого должно быть своё description, и ароматы со вкусами обязательно указывай путями из справочников — разбиение на проливы не отменяет разбора оттенков.
...
const userText = (source) => {
  ...
  const lines = source.map(({ brewingNumber, whole, transcript }) => {
    // An imported recording of the whole tasting. It is emphatically NOT a note
    // about one пролив, and mislabelling it «[О чае]» measurably cost the
    // descriptors: the model read it as one general remark and returned no
    // aromas or tastes at all.
    if (whole) return `[Вся сессия] ${transcript}`;
    return Number(brewingNumber) > 0
